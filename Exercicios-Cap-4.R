# Lista de Exercícios - Capítulo 4 

# Obs: Caso tenha problemas com a acentuação, consulte este link:
# https://support.rstudio.com/hc/en-us/articles/200532197-Character-Encoding

# Configurando o diretório de trabalho
# Coloque entre aspas o diretório de trabalho que você está usando no seu computador
# Não use diretórios com espaço no nome
setwd("C:/FCD/BigDataRAzure/Cap04")
getwd()

# Exercicio 1 - Crie uma função que receba os dois vetores abaixo como parâmetro, 
# converta-os em um dataframe e imprima no console
vec1 <- (10:13)
vec2 <- c("a", "b", "c", "d")

recebe_2_Param <- function(param1, param2){
  df <- cbind.data.frame(param1, param2)
  return(df)
}

recebe_2_Param(vec1,vec2)

# Exercicio 2 - Crie uma matriz com 4 linhas e 4 colunas preenchida com 
# números inteiros e calcule a média de cada linha
matriz <- matrix(c(1:16), nrow = 4, ncol = 4)
rowMeans(matriz)
apply(mat, 1 , mean) #solução possível / alternativa (1) op. por nível de linha

# Exercicio 3 - Considere o dataframe abaixo. 
# Calcule a média por disciplina e depois calcule a média de apenas uma disciplina
escola <- data.frame(Aluno = c('Alan', 'Alice', 'Alana', 'Aline', 'Alex', 'Ajay'),
                     Matematica = c(90, 80, 85, 87, 56, 79),
                     Geografia = c(100, 78, 86, 90, 98, 67),
                     Quimica = c(76, 56, 89, 90, 100, 87))

colMeans(escola[2:4])
colMeans(escola['Quimica'])

apply(escola[, c(2,3,4)], 2, mean) #solução possível / alternativa
apply(escola[,c(2),drop=F],2,mean) #verificar arquivo solução professor para mais informações

# Exercicio 4 - Cria uma lista com 3 elementos, todos numéricos 
# e calcule a soma de todos os elementos da lista
lista_num <- list(1:3)
lista_num
sum(lista_num[[1]])

do.call(sum, lista_num) # solução possível / alternativa

# Exercicio 5 - Transforme a lista anterior um vetor
vetor <- unlist(lista_num)
vetor

# Exercicio 6 - Considere a string abaixo. Substitua a palavra "textos" por "frases"
str <- c("Expressoes", "regulares", "em linguagem R", 
         "permitem a busca de padroes", "e exploracao de textos",
         "podemos buscar padroes em digitos",
         "como por exemplo",
         "10992451280")

# gsub a todas as correspondencias - sub apenas a primeira
gsub("textos", "frases", str, ignore.case = T)

# Exercicio 7 - Usando o dataset mtcars, crie um grafico com ggplot do tipo 
# scatter plot. Use as colunas disp e mpg nos eixos x e y respectivamente
data(mtcars)
?mtcars
library(ggplot2)
#qplot(x=mtcars$disp, y=mtcars$mpg, xlab='Disp', ylab='MPG', col='salmon', geom = 'point')

camada1 <- geom_point(mapping = aes(x=disp, y=mpg),
                      data = mtcars,
                      color='darkblue',
                      size=3)

ggplot()+ camada1 + xlab("Displacement") + ylab("Miles/(US) gallon")


# Exercicio 8 - Considere a matriz abaixo.
# Crie um bar plot que represente os dados em barras individuais.
mat1 <- matrix(c(652,1537,598,242,36,46,38,21,218,327,106,67), nrow = 3, byrow = T)

# Converter e nomear colunas
data_frame <- as.data.frame(mat1)
colnames(data_frame) <- c('col1', 'col2', 'col3', 'col4')

# Transformar o data frame em formato longo
data_long <- reshape2::melt(data_frame)

# Criar gráfico de barras individuais
ggplot(data_long, aes(x = variable, y = value, fill = factor(variable))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Grupos", y = "Valores", title = "Gráfico de Barras Individuais") +
  theme_minimal()

# PODERIA TER UTILIZADO O PCT BASICO, a solução seria simplesmente
barplot(mat1, beside = T)

# Exercício 9 - Qual o erro do código abaixo?
data(diamonds)
ggplot(data = diamonds, aes(x = price, group = cut, fill = cut)) + 
  geom_density(adjust = 1.5)


# Exercício 10 - Qual o erro do código abaixo?
?mtcars
ggplot(mtcars, aes(x = as.factor(cyl), fill = as.factor(cyl))) + 
  geom_bar() +
  labs(fill = "cyl") + xlab("N° de Cilindros") + ylab("QTD") +
  theme(legend.position = "top")
