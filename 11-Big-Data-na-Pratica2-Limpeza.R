...
dados_salario <- read.csv("datasets_originais/dados_desigualdade_salarial_harvard.csv") #separado por virgula
View(dados_salario)

# Carregando dados do IMDB: 
dados_IMDB <- read_tsv("datasets_originais/dados_imdb.tsv") #separado por tab
View(dados_IMDB)
...
genero <- dados_IMDB[,-c(1, 4:8)] #filtro slicing em R (antes da virgula linha depois col), o menos quer dizer tudo menos as colunas indicadas!!!
View(genero)
names(genero)[names(genero) == 'primaryTitle'] <- 'show_title' #names permite renomear a col
View(genero)
...
top10sunburst <- rbind(top10add, top10sunburst) #une dataframes por linha / cbind seria por cols
